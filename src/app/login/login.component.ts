    ...
    this.authService.login(username, password).subscribe({
      next: (success) => {
        ...
        if (success) {
           ...
           // Navigate to dashboard after a short delay to allow toast visibility
      ...
      error: (err) => {
        this.loading = false;
        this.messageService.add({ severity: 'error', summary: 'Login Failed', detail: err.message || 'Incorrect username or password.' });
      }
    });
  }
}
